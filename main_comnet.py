...
out_put1=Reshape((2,int(payloadBits_per_OFDM/2)))(out_put1)#The way of 'Reshape' works is row first,but we need column first way
out_put1=Permute((2,1))(out_put1)

def plural_division(h_ls):
    ...
    real_result=(real_input_Data*real_input_hls+imag_input_Data*imag_input_hls)/(real_input_hls**2+imag_input_hls**2)
    imag_result=(imag_input_Data*real_input_hls-real_input_Data*imag_input_hls)/(real_input_hls**2+imag_input_hls**2)
    return concatenate([real_result,imag_result])

x_zf=Lambda(plural_division)(out_put1)
...
if train_flag==True:
    ...
    #save h_predict
    if save_h_pred==True:
        h_layer=Model(inputs=model.input,outputs=model.get_layer('batch_normalization_3').output)#output result of h_pred
